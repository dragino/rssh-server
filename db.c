...
#define RSSH_DB_FILE       "/var/rsshdb.sqlite3"
...
DEFINE_SQL_STATEMENT(put_stmt, "INSERT OR REPLACE INTO rsshtb (key, value) VALUES (?, ?)")
DEFINE_SQL_STATEMENT(get_stmt, "SELECT value FROM rsshtb WHERE key=?")
DEFINE_SQL_STATEMENT(del_stmt, "DELETE FROM rsshtb WHERE key=?")
DEFINE_SQL_STATEMENT(deltree_stmt, "DELETE FROM rsshtb WHERE key || '/' LIKE ? || '/' || '%'")
DEFINE_SQL_STATEMENT(deltree_all_stmt, "DELETE FROM rsshtb")
DEFINE_SQL_STATEMENT(gettree_stmt, "SELECT key, value FROM rsshtb WHERE key || '/' LIKE ? || '/' || '%' ORDER BY key")
DEFINE_SQL_STATEMENT(gettree_all_stmt, "SELECT key, value FROM rsshtb ORDER BY key")
DEFINE_SQL_STATEMENT(showkey_stmt, "SELECT key, value FROM rsshtb WHERE key LIKE '%' || '/' || ? ORDER BY key")
DEFINE_SQL_STATEMENT(gettree_prefix_stmt, "SELECT key, value FROM rsshtb WHERE key > ?1 AND key <= ?1 || X'ffff'")
...
static inline int lgw_strlen_zero(const char *s)
{
        return (!s || (*s == '\0'));
}
...
{
	/* Don't initialize create_rsshdb_statement here as the rsshdb table needs to exist
...
{
	pthread_mutex_lock(&mx_dblock);
	if (sqlite3_open(RSSH_DB_FILE, &DB) != SQLITE_OK) {
		lgw_log(LOG_WARNING, "WARNING~ [db] Unable to open LGW database '%s': %s\n", RSSH_DB_FILE, sqlite3_errmsg(DB));
		sqlite3_close(DB);
		pthread_mutex_unlock(&mx_dblock);
		return -1;
	}
...
/*!
 * \internal
 * \brief Signal the rsshdb sync thread to do its thing.
 *
...
/*!
 * \internal
 * \brief rsshdb sync thread
 *
 * This thread is in charge of syncing rsshdb to disk after a change.
